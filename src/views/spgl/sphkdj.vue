<template>
  <div class="smain">
    ...
    <el-form :inline="true" :model="formInline" size="small" class="demo-form-inline">
      ...
      <el-form-item>
        <el-cascader ref="options1" placeholder="请选择品牌类型" :options="options1" v-model="formInline.flid" :show-all-levels="false" clearable> </el-cascader>
      </el-form-item>
      <el-form-item>
    ...
    <el-dialog class="upimage" :title="row.title" :visible.sync="uploadimagehas" width="56%" style="padding-bottom: 5%;">
      <Upsphkdj :splist="splist" :listrow="row" :uploadimagehas="uploadimagehas" @dialog1Changed="childchanged($event)"></Upsphkdj>
    </el-dialog>
    <!-- 分页 -->
...
import Upsphkdj from './components/Upsphkdj'
...
export default {
  ...
  methods: {
    ...
    onloadtable1() { //查询
      ...
      request({ url: 'mall/kchc/searchkchc.do', method: 'post', data: txmxcxData })
        .then(response => {
    ...
    onloadtable3(val) { //查询所有商品
      request({ url: 'mall/ckxq/getckxq.do', method: 'post', data: {} })
        .then((response) => {
    ...
    uploadimag(row, add) { // 添加、修改 库存
      if (add) {
        this.row = {};
        this.row.btn = "添加";
        this.row.title = "添加回库登记";
      } else {
        this.row = row;
        this.row.showzt = true;
        this.row.btn = "修改";
        this.row.title = "修改回库登记";
